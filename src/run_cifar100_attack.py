...
class Cifar100Attack(object):
    """CIFAR100, Attackerclass.
    Parameters
    ----------
    result_dir : str
    ...
    def load_model(self):
        ...
        if model_name == "TRADES":
            ...
            self.model = robustbench_load_model(
                model_name, dataset="cifar100", threat_model="Linf"
            )
    ...
    def load_dataset(self):
        ...
        if dataset_name == "cifar100":
            x_test, y_test = load_cifar100(n_examples=self.config.n_examples)
        else:
...
def main(
    ...
    cifar100_attack = Cifar100Attack(
    ...
    if type(idx) is str:
        idx = int(idx)
    adv_example, logs, save_log_path, staticsfilepath = cifar100_attack.run(
        idx,
        bs,
        EXP=EXP,
        export=export,
